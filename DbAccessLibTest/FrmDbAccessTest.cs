...
namespace DbAccessLibTest
{
    public partial class FrmDbAccessTest : Form
    {
        ...
        private void btnExecute_Click(object sender, EventArgs e)
        {
            ...
            _isSingleton = ckbLibSingleton.Checked;
            _totalElapsed = 0;
            _workerInfo = new List<Worker>();
            _testLibClassPath = "DbAccessLibTest.Test." + cmbTestLib.Text;
            _testInstance = _isSingleton ?
                (ITest)(_self.GetType(_testLibClassPath).GetProperty("Instance").GetValue(null, null)) :
                (ITest)_self.CreateInstance(_testLibClassPath);
            if (_testInstance == null)
            {
                UpdateMessage("创建测试类实体失败，" + _testLibClassPath);
                return;
            }
            _testInstance.Init();
        ...
        private bool UseTask(int index, Action<ITest, int> testExecute)
        {
            var task = new Task<object>
                (
                    threadIndex => TestWork(threadIndex, testExecute, GetTestInstance())
                    , index
        ...
        private void UpdateChart()
        {
            ...
            else
            {
                ...
                UpdateMessage(string.Format("平均值-最高值-最低值\n{0}-{1}-{2}", Math.Round(_workerInfo.Average(o => o.TotalElapsed)), _workerInfo.Max(o => o.TotalElapsed), _workerInfo.Min(o => o.TotalElapsed)));
            }
        }
        /// <summary>
        /// 更新信息
